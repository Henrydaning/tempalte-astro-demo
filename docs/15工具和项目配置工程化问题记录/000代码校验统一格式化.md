...
第一步：根据当前的 lint-staged 内容配置
    "lint": "eslint . --fix",//注意配置这里.，可以换成src文件夹，这样只会校验src文件夹内容
...
pre-commit.sh中增加，将原来的npm  test修改为npm run  lint-staged

### （9）有时候一个项目安装增加husky+lint-staged后不生效
将一个js文件简单做一些修改，这样可以方便校验。
有时候，有缓存的影响，导致不生效。
